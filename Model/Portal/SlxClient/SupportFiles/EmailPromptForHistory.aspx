...
<head id="Head1" runat="server">
    ...
    <script pin="pin" type="text/javascript">
        ...
        var bCanceled = true;
        var sFollowUpInfo = null;

        function setCanceled(value) {
            bCanceled = value;
        }

        function setFollowUpInfo(json) {
            sFollowUpInfo = json;
        }
        ...
        function unload() {
            ...
            if (!bCanceled) {
                var oResult = $get("Description_Text");
                if (oResult != null) {
                    sResult = oResult.value;
                }
                if (!Ext.isEmpty(sFollowUpInfo)) {
                    sResult = sFollowUpInfo;
                }
            }

            if (Ext.isString(sResult) || Ext.isObject(sResult)) {
                if (IsSlxModalDialog()) {
            ...
            if (IsSlxModalDialog()) {
                var iModalResult = (bCanceled) ? 2 : 1;
                window.external.SlxModalDialog.CloseDialog(iModalResult);
            }
            else {
                // window.close();
                // Handled in EmailPromptForHistory.aspx.cs.
            }
        }
    </script>
...
<td >  
    ...
    <asp:Button runat="server" ID="btnOk" Text="OK" ToolTip="Complete Activity" OnClientClick="" meta:resourcekey="Ok_rsc" />
    <asp:Button runat="server" ID="btnCancel" Text="Cancel" ToolTip="Cancel" OnClientClick="" meta:resourcekey="btnCancelResource1" />
