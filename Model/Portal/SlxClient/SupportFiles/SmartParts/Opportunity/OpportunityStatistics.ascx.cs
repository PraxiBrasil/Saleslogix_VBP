...
public partial class SmartParts_OpportunityStatistics : UserControl, ISmartPartInfoProvider
{
    ...
    private void GetPotentialValues(DataTable oppGroup)
    {
        ...
        {
            Double thisSalesPotential = 0.0, thisCloseProbability = 0.0;
            Double.TryParse(dr[0].ToString(), out thisSalesPotential);
            Double.TryParse(dr[1].ToString(), out thisCloseProbability);
            salesPotential = salesPotential + thisSalesPotential;
            if (thisCloseProbability != 0)
                weighted = weighted + thisSalesPotential * (thisCloseProbability / 100);
        }
    ...
    private void GetAverageCloseProb(DataTable oppGroup)
    {
        ...
        foreach (DataRow dr in oppGroup.Rows)
        {
            Double thisCloseProbability = 0.0;
            Double.TryParse(dr[1].ToString(), out thisCloseProbability);
            closeProb = closeProb + thisCloseProbability;
        }
