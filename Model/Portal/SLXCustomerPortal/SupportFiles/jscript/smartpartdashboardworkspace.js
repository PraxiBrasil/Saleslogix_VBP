...
function setupPortal() {
    ...
    //Ext.QuickTips.init(); -- moved to general.js
};
...
Sage.SalesLogix.Dashboard.prototype.createCellObject = function(name, portalNum, column) {
    ...
    return {
        ...
        //html: Ext.get(name + "contents").dom.innerHTML //if we don't copy the html then we can't revert, but if we do then we break .net smartparts
        contentEl: name + "contents", //switch this for the line above to have ext revert and multiple copies work.
        autoDestroy: false
    };
}
    ...
    var tcmenu = new Ext.menu.Menu({ items: [
        ...
        new Ext.menu.Item({ text: DashboardResource.Add_Content, handler: function() {
            var win = new Ext.Window({ title: DashboardResource.Add_New_Content,
                ...
                autoScroll: true,
                buttons: [{ text: DashboardResource.Ok, handler: function(t, e) { curDashboard.saveState(); win.close(); } }
